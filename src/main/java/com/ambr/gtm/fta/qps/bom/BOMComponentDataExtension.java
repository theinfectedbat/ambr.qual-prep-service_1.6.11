 ...
 *****************************************************************************************
 */
@Table(name = "mdi_bom_comp_de")
public class BOMComponentDataExtension 
	implements Serializable, DataRecordValueMapInterface
{
     ...
     *************************************************************************************
     */
	public BOMComponentDataExtension()
		throws Exception
	{
		...
		throws Exception 
	{
		...
		if (this.repos == null) {
			aList = new ArrayList<>(new DataRecordUtility<>(BOMComponentDataExtension.class, true).getColumnNames());
			...
			aList.addAll(new DataRecordUtility<BOMComponentDataExtension>(BOMComponentDataExtension.class, true).getColumnNames());
		}
	...
	public String getDescription() {
		return MessageFormat.format("BOM.{0}.COMP.{1}", this.alt_key_bom, this.alt_key_comp);
	}
	 ...
	 */
	@JsonIgnore
	void setComponent(BOMComponent theBOMComponent)
		throws Exception
	{
		...
		throws Exception 
	{
		...
		if ("alt_key_bom".equalsIgnoreCase(theColumnName)) {
			...
			this.alt_key_comp = ((Number)theValue).longValue();
		}
