 ...
 */
@Configuration
public class QualTXDetailUniversePostStartupInitializer implements Runnable
{
	static Logger	logger = LogManager.getLogger(QualTXDetailUniversePostStartupInitializer.class);
	...
	@Autowired private QualTXDetailUniverse				universe;
     ...
     *************************************************************************************
     */
	public QualTXDetailUniversePostStartupInitializer()
		throws Exception
	{
	}
		...
		throws Exception
	{
		...
		if ("Y".equalsIgnoreCase(this.propertyResolver.getPropertyValue(QualTXDetailUniverseProperties.UNIVERSE_ENABLED, "N")) == false) {
			// We don't need to initialize this JVM with a functional BOM Universe
			MessageFormatter.info(logger, "completeInitialization", "Qual TX Detail Universe does not require initialization.");
			return;
		}
		...
		aMemMax = Integer.valueOf(this.propertyResolver.getPropertyValue(QualTXDetailUniverseProperties.UNIVERSE_PARTITION_MEM_MAX, String.valueOf(this.universe.getMemoryMax())));
		aMemMin = Integer.valueOf(this.propertyResolver.getPropertyValue(QualTXDetailUniverseProperties.UNIVERSE_PARTITION_MEM_MIN, String.valueOf(this.universe.getMemoryMin())));
		...
		try {
			...
			MessageFormatter.error(logger, "completeInitialization", e, "Qual TX Detail Universe failed to initialize.");
		}
	}
}
