 ...
 */
@Configuration
public class GPMSourceIVAUniversePostStartupInitializer implements Runnable
{
	static Logger						logger = LogManager.getLogger(GPMSourceIVAUniversePostStartupInitializer.class);
     ...
     *************************************************************************************
     */
	public GPMSourceIVAUniversePostStartupInitializer()
		throws Exception
	{
	}
		...
		throws Exception
	{
		...
		if ("Y".equalsIgnoreCase(this.propertyResolver.getPropertyValue(GPMSourceIVAUniverseProperties.UNIVERSE_ENABLED, "N")) == false) {
			// We don't need to initialize this JVM with a functional GPM Source IVA Universe
			MessageFormatter.info(logger, "completeInitialization", "GPM Source IVA Universe does not require initialization.");
			return;
		}
		...
		aMemMax = Integer.valueOf(this.propertyResolver.getPropertyValue(GPMSourceIVAUniverseProperties.UNIVERSE_PARTITION_MEM_MAX, String.valueOf(this.gpmSrcIVAUniverse.getMemoryMax())));
		aMemMin = Integer.valueOf(this.propertyResolver.getPropertyValue(GPMSourceIVAUniverseProperties.UNIVERSE_PARTITION_MEM_MIN, String.valueOf(this.gpmSrcIVAUniverse.getMemoryMin())));
		...
		this.gpmSrcIVAUniverse.setMemoryMax(aMemMax);
		this.gpmSrcIVAUniverse.setMemoryMin(aMemMin);
		this.gpmSrcIVAUniverse.setDBURL(this.dataSrcProperties.getUrl());
		this.gpmSrcIVAUniverse.setDBUserName(this.dataSrcProperties.getUsername());
		this.gpmSrcIVAUniverse.setDBPassword(this.dataSrcProperties.getPassword());
		this.gpmSrcIVAUniverse.setTargetSchema(aTargetSchema);
		this.gpmSrcIVAUniverse.setFetchSize(aFetchSize);
		this.gpmSrcIVAUniverse.setMaxCursorDepth(aMaxQueueDepth);
		this.gpmSrcIVAUniverse.setServiceJarFileName(aQPSJarFile);
		this.gpmSrcIVAUniverse.setPartitionCount(aPartitionCount);
		this.gpmSrcIVAUniverse.setServerPort(aServerPort);
		this.gpmSrcIVAUniverse.setPropertyResolver(this.propertyResolver);
		...
		try {
			this.gpmSrcIVAUniverse.startup();
			MessageFormatter.info(logger, "completeInitialization", "complete.");
		}
		catch (Exception e) {
			MessageFormatter.error(logger, "completeInitialization", e, "GPM Source IVA Universe failed to initialize.");
		}
	}
}
